...
boolean Simple_Rd_03D::begin(uint8_t _serial_mux){
    ...
    unsigned char dataMulti[] = {0xFD, 0xFC, 0xFB, 0xFA, 0x02, 0x00, 0x90, 0x00, 0x04, 0x03, 0x02, 0x01};
    Serial1.write(dataMulti, sizeof(dataMulti));
    Serial1.flush();
    unsigned long startTime = millis();
    //expecting response: FD FC FB FA 04 00 90 01 01 00 04 03 02 01
    unsigned char dataResponseExpected[] = {0xFD, 0xFC, 0xFB, 0xFA, 0x04, 0x00, 0x90, 0x01, 0x01, 0x00, 0x04, 0x03, 0x02, 0x01};
...
void Simple_Rd_03D::disableRadarStreaming(){
    ...
    unsigned char dataMulti[] = {0xFD, 0xFC, 0xFB, 0xFA, 0x04, 0x00, 0xFF, 0x00, 0x01, 0x00, 0x04, 0x03, 0x02, 0x01};
    Serial1.write(dataMulti, sizeof(dataMulti));
    Serial1.flush();
}
...
void Simple_Rd_03D::enableRadarStreaming(){
    ...
    unsigned char dataMulti[] = {0xFD, 0xFC, 0xFB, 0xFA, 0x02, 0x00, 0xFE, 0x00, 0x04, 0x03, 0x02, 0x01};
    Serial1.write(dataMulti, sizeof(dataMulti));
    Serial1.flush();
}
    ...
    while ((int)(millis() - startTime) < timeout) {
        if (Serial1.available()) {
            ...
            if (radarDataLength>= 30) {
                if (radarRecvBuffer[(radarRecvBufferIndex+0)%30] == 0xAA && radarRecvBuffer[(radarRecvBufferIndex+1)%30] == 0xFF && radarRecvBuffer[(radarRecvBufferIndex+2)%30] == 0x03 && radarRecvBuffer[(radarRecvBufferIndex+3)%30] == 0x00) {
                    // check if the last two bytes are 0x55 and 0xCC
                    if (radarRecvBuffer[(radarRecvBufferIndex+28)%30] == 0x55 && radarRecvBuffer[(radarRecvBufferIndex+29)%30] == 0xCC) {
                        int detectedObjectCount = 0;
                        for (int i = 0; i < 3; i++){
                            int offset = (radarRecvBufferIndex + 4 + i * 8) % 30;
                            // we are going to check 4 of the 8 bytes, each 2 bytes is little endian int16 type
                            // x is mm, y is mm, speed is cm/s, resolution is mm
                            // need more optimization

                            int16_t x = (radarRecvBuffer[(offset+1)%30] << 8) | radarRecvBuffer[(offset+0)%30];
                            if (x<0) x = -32768 - x;
                            int16_t y = (radarRecvBuffer[(offset+3)%30] << 8) | radarRecvBuffer[(offset+2)%30];
                            if (y<0) y= -32768 - y;
                            int16_t speed = (radarRecvBuffer[(offset+5)%30] << 8) | radarRecvBuffer[(offset+4)%30];
                            if (speed<0) speed= -32768 - speed;
                            uint16_t resolution = (radarRecvBuffer[(offset+7)%30] << 8) | radarRecvBuffer[(offset+6)%30];
                            // check if the x, y, speed are all 0
                            if (x == 0 && y == 0 && speed == 0){
                                // no target
                                //Serial.println("No target");
                            }else{
                                radarObject_t *radarObjectPtr = &radarObject[detectedObjectCount];
                                radarObjectPtr->x = x;
                                radarObjectPtr->y = y;
