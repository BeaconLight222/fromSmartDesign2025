...
boolean Simple_MLX90640::begin(uint8_t _i2c_addr) {
  ...
  int readNumber =
      MLX90640_I2CRead(i2c_addr, MLX90640_DEVICEID1, 3, serialNumber);
  ...
  _params.alpha =
      (uint16_t *)(flashSwap +
                   0); // Assign the alpha array to the params structure
  _params.offset =
      (int16_t *)(flashSwap +
                  768 * 2); // Assign the offset array to the params structure
  _params.kta =
      (int8_t *)(flashSwap + 768 * 2 +
                 768 * 2); // Assign the kta array to the params structure
  _params.kv = (int8_t *)(flashSwap + 768 * 2 + 768 * 2 +
                          768); // Assign the kv array to the params structure
  MLX90640_ExtractParameters(eeMLX90640, &_params);

  int irSensorBaseAddress =
      0x08204000; // use Wi-Fi Fast Connect profile space, we are not using it
  bool contentSame = (memcmp((void *)irSensorBaseAddress, (void *)flashSwap,
                             sizeof(flashSwap)) == 0);
  if (!contentSame) {
    Serial.println("IR sensor EEPROM data is not in flash memory or different, "
                   "writing to flash...");
    ...
    flash_erase_sector(NULL, (uint32_t)(flashAddress + 0x1000));
    flash_stream_write(NULL, (uint32_t)flashAddress, 4096, flashSwap);
    flash_stream_write(NULL, (uint32_t)(flashAddress + 0x1000),
                       sizeof(flashSwap) - 4096, flashSwap + 4096);
  } else {
    Serial.println("IR sensor EEPROM data is already in flash memory and same, "
                   "skipping write.");
  }

  _params.alpha =
      (uint16_t *)(irSensorBaseAddress +
                   0x00); // Assign the alpha array to the params structure
  _params.offset =
      (int16_t *)(irSensorBaseAddress + 0x00 +
                  768 * 2); // Assign the offset array to the params structure
  _params.kta =
      (int8_t *)(irSensorBaseAddress + 0x00 + 768 * 2 +
                 768 * 2); // Assign the kta array to the params structure
  _params.kv = (int8_t *)(irSensorBaseAddress + 0x00 + 768 * 2 + 768 * 2 +
                          768); // Assign the kv array to the params structure
...
int Simple_MLX90640::MLX90640_I2CRead(uint8_t slaveAddr, uint16_t startAddress,
  ...
  while (nMemAddressRead > 0) {
    ...
    for (uint16_t i = 0; i < toRead16; i++) {
      ...
      data[i] =
          (Wire.read() << 8) | Wire.read(); // Read two bytes and combine them
    }
...
int Simple_MLX90640::MLX90640_ExtractParameters(uint16_t *eeData,
...
int Simple_MLX90640::MLX90640_SetResolution(uint8_t slaveAddr,
                                            uint8_t resolution) {
  uint16_t controlRegister1;
  int value;
  int error;
...
int Simple_MLX90640::MLX90640_SetRefreshRate(uint8_t slaveAddr,
                                             uint8_t refreshRate) {
  uint16_t controlRegister1;
  int value;
  int error;
...
int Simple_MLX90640::MLX90640_GetFrameData(uint8_t slaveAddr,
                                           uint16_t *frameData) {
...
float Simple_MLX90640::MLX90640_GetTa(uint16_t *frameData,
...
float Simple_MLX90640::MLX90640_GetVdd(uint16_t *frameData,
                                       const paramsMLX90640 *params) {
  float vdd;
...
void Simple_MLX90640::MLX90640_CalculateTo(uint16_t *frameData,
                                           const paramsMLX90640 *params,
