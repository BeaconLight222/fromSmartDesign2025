...
BLEClientPatched::BLEClientPatched() {
}
...
bool BLEClientPatched::connected() {
    return BLEPatched.connected(_connId);
}

void BLEClientPatched::discoverServices() {
...
bool BLEClientPatched::discoveryDone() {
    return (_serviceDiscState == DISC_DONE);
}

void BLEClientPatched::printServices() {
    ...
    BLERemoteDescriptorPatched* pdesc;
...
BLERemoteService* BLEClientPatched::getService(const char* uuid) {
    return getService(BLEUUIDPatched(uuid));
}

BLERemoteService* BLEClientPatched::getService(BLEUUIDPatched uuid) {
    if (_serviceDiscState != DISC_DONE) {
        printf("Conn ID %d client error: Discover services first \n", _connId);
        return nullptr;
    } else if (_serviceCount == 0) {
...
uint8_t BLEClientPatched::getConnId() {
    return _connId;
}

T_CLIENT_ID BLEClientPatched::getClientId() {
    return _clientId;
}

void BLEClientPatched::setDisconnectCallback(void (*fCallback) (BLEClientPatched* client)) {
    _pDiscCB = fCallback;
}

void BLEClientPatched::clientDiscoverStateCallbackDefault(uint8_t conn_id, T_DISCOVERY_STATE discovery_state) {
    if (conn_id != _connId) {
...
void BLEClientPatched::clientDiscoverResultCallbackDefault(uint8_t conn_id, T_DISCOVERY_RESULT_TYPE result_type, T_DISCOVERY_RESULT_DATA result_data) {
    if (conn_id != _connId) {
        ...
        switch (result_type) {
            ...
            case DISC_RESULT_ALL_SRV_UUID128:
                if (result_type == DISC_RESULT_ALL_SRV_UUID16) {
                    ...
                    uuid = BLEUUIDPatched((uint8_t*)(&(result_data.p_srv_uuid16_disc_data->uuid16)), 2);
                    ...
                    uuid = BLEUUIDPatched(result_data.p_srv_uuid128_disc_data->uuid128, 16);
                }
            ...
            case DISC_RESULT_CHAR_UUID128:
                if (result_type == DISC_RESULT_CHAR_UUID16) {
                    ...
                    uuid = BLEUUIDPatched((uint8_t*)(&(result_data.p_char_uuid16_disc_data->uuid16)), 2);
                    ...
                    uuid = BLEUUIDPatched(result_data.p_char_uuid128_disc_data->uuid128, 16);
                }
                for (uint8_t i = 0; i < _serviceCount; i++) {
            ...
            case DISC_RESULT_CHAR_DESC_UUID128:
                if (result_type == DISC_RESULT_CHAR_DESC_UUID16) {
                    ...
                    uuid = BLEUUIDPatched((uint8_t*)(&(result_data.p_char_desc_uuid16_disc_data->uuid16)), 2);
                } else if (result_type == DISC_RESULT_CHAR_DESC_UUID128) {
                    handle_decl = result_data.p_char_desc_uuid128_disc_data->handle;
                    uuid = BLEUUIDPatched(result_data.p_char_desc_uuid128_disc_data->uuid128, 16);
                }
                ...
                for (uint8_t i = 0; i < _serviceCount; i++) {
                    ...
                    if ((handle_decl > _servicePtrList[i]->_handleStart) && (handle_decl <= _servicePtrList[i]->_handleEnd)) {
                        ...
                        if ((handle_decl != pchar->_declarationHandle) && (handle_decl != pchar->_valueHandle)) {
                            ...
                            BLERemoteDescriptorPatched* newDesc = new BLERemoteDescriptorPatched(uuid);
...
void BLEClientPatched::clientReadResultCallbackDefault(uint8_t conn_id, uint16_t cause, uint16_t handle, uint16_t value_size, uint8_t *p_value) {
    if (conn_id != _connId) {
...
void BLEClientPatched::clientWriteResultCallbackDefault(uint8_t conn_id, T_GATT_WRITE_TYPE type, uint16_t handle, uint16_t cause, uint8_t credits) {
    if (conn_id != _connId) {
...
T_APP_RESULT BLEClientPatched::clientNotifyIndicateCallbackDefault(uint8_t conn_id, bool notify, uint16_t handle, uint16_t value_size, uint8_t *p_value) {
...
void BLEClientPatched::clientDisconnectCallbackDefault(uint8_t conn_id) {
    if (conn_id != _connId) {
