...
uint8_t BLEScanPatched::_scanProcessing = 0;

BLEScanPatched::BLEScanPatched() {
    _scanProcessing = 0;
}

void BLEScanPatched::updateScanParams() {
...
void BLEScanPatched::startScan(uint32_t scanDuration_ms) {
    startScan();
...
void BLEScanPatched::setScanMode(uint8_t scanMode) {
...
void BLEScanPatched::setScanInterval(uint16_t scanInt_ms) {
    if ((scanInt_ms >= 3) && (scanInt_ms <= 10240)) {
        _scanInterval = (scanInt_ms*1000/625);
    }
}

void BLEScanPatched::setScanWindow(uint16_t scanWindow_ms) {
...
void BLEScanPatched::setScanDuplicateFilter(bool dupeFilter) {
    if (dupeFilter) {
...
bool BLEScanPatched::scanInProgress() {
    if (_scanProcessing) {
        return true;
    }
    return false;
...
void BLEScanPatched::printScanInfo(T_LE_CB_DATA *p_data) {
    char adv_type[20];
