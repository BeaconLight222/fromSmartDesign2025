...
BLEDevicePatched BLEPatched;

uint8_t BLEDevicePatched::_bleState = 0;
BLEAdvertPatched* BLEDevicePatched::_pBLEAdvert = nullptr;
BLEScanPatched* BLEDevicePatched::_pBLEScan = nullptr;
void (*BLEDevicePatched::_pScanCB)(T_LE_CB_DATA*) = nullptr;
BLEConnectPatched* BLEDevicePatched::_pBLEConn = nullptr;
BLESecurityPatched* BLEDevicePatched::_pBLESecurity = nullptr;
BLEServicePatched* BLEDevicePatched::_servicePtrList[BLE_MAX_SERVICE_COUNT] = {};
uint8_t BLEDevicePatched::_serviceCount = 0;
BLEClientPatched* BLEDevicePatched::_clientPtrList[BLE_CENTRAL_APP_MAX_LINKS] = {};
void *BLEDevicePatched::_appTaskHandle = NULL;   //!< main task handle
void *BLEDevicePatched::_evtQueueHandle = NULL;  //!< Event queue handle
void *BLEDevicePatched::_ioQueueHandle = NULL;   //!< IO queue handle
T_GAP_DEV_STATE BLEDevicePatched::_gapDevState = {0, 0, 0, 0, 0};
T_GAP_CONN_STATE BLEDevicePatched::_gapConnState = GAP_CONN_STATE_DISCONNECTED;
T_APP_LINK BLEDevicePatched::_bleCentralAppLinkTable[BLE_CENTRAL_APP_MAX_LINKS];
uint8_t BLEDevicePatched::all_phys = GAP_PHYS_PREFER_2M_BIT;
uint8_t BLEDevicePatched::tx_phys = GAP_PHYS_PREFER_2M_BIT;
uint8_t BLEDevicePatched::rx_phys = GAP_PHYS_PREFER_2M_BIT;
T_GAP_PHYS_OPTIONS BLEDevicePatched::phy_options = GAP_PHYS_OPTIONS_CODED_PREFER_NO;

extern const u8 ftl_phy_page_num;
extern const u32 ftl_phy_page_start_addr;

BLEDevicePatched::BLEDevicePatched() {
}

// allocate memory and low level resources for BT
// call this method before doing anything BT related
void BLEDevicePatched::init() {
    ...
    if (new_state.gap_init_state == GAP_INIT_STATE_STACK_READY) {
        ...
        //do not do it in 4M flash firmware space!
        ftl_init(0x00202000, 1);
        bte_init();
    }
}

// free up memory and low level resources used by BT
void BLEDevicePatched::deinit() {
    T_GAP_DEV_STATE new_state;
    le_get_gap_param(GAP_PARAM_DEV_STATE , &new_state);
    if (new_state.gap_init_state != GAP_INIT_STATE_STACK_READY) {
        printf("BT Stack is not running\r\n");
    } else {
        BLEDevicePatched::end();
        bte_deinit();
        bt_trace_uninit();
...
bool BLEDevicePatched::connected(uint8_t connId) {
...
void BLEDevicePatched::setDeviceName(String devName) {
...
void BLEDevicePatched::setDeviceAppearance(uint16_t devAppearance) {
    _appearance = devAppearance;
}

BLEAdvertPatched* BLEDevicePatched::configAdvert() {
    if (_pBLEAdvert == nullptr) {
        _pBLEAdvert = new BLEAdvertPatched();
    }
    return (_pBLEAdvert);
}

BLEScanPatched* BLEDevicePatched::configScan() {
    if (_pBLEScan == nullptr) {
        _pBLEScan = new BLEScanPatched();
    }
    return (_pBLEScan);
}

BLEConnectPatched* BLEDevicePatched::configConnection() {
    if(_pBLEConn == nullptr) {
        _pBLEConn = new BLEConnectPatched();
    }
    return (_pBLEConn);
}

BLESecurityPatched* BLEDevicePatched::configSecurity() {
    if(_pBLESecurity == nullptr) {
        _pBLESecurity = new BLESecurityPatched();
    }
    return (_pBLESecurity);
}

void BLEDevicePatched::setScanCallback(void (*scanCB)(T_LE_CB_DATA*)) {
    _pScanCB = scanCB;
}

// starts the BLE stack to operate as a central device
// note: central devices should not advertise to other devices
void BLEDevicePatched::beginCentral(uint8_t connCount) {
    T_GAP_DEV_STATE new_state;
    if (_bleState != 0) {
...
void BLEDevicePatched::beginPeripheral() {
    T_GAP_DEV_STATE new_state;
    if (_bleState != 0) {
...
void BLEDevicePatched::configServer(uint8_t maxServiceCount) {
...
void BLEDevicePatched::configClient() {
...
void BLEDevicePatched::addService(BLEServicePatched& newService) {
...
BLEClientPatched* BLEDevicePatched::addClient(uint8_t connId) {
    BLEClientPatched* newClient = nullptr;
    ...
    newClient = new BLEClientPatched();
...
void BLEDevicePatched::getLocalAddr(uint8_t (&addr)[GAP_BD_ADDR_LEN]) {
    uint8_t  btaddr[GAP_BD_ADDR_LEN] = {0};
    gap_get_param(GAP_PARAM_BD_ADDR, btaddr);
    memcpy(addr, btaddr, sizeof(btaddr));
}

void BLEDevicePatched::BLEMainTask(void *p_param) {
    (void)p_param;
    uint8_t event;
