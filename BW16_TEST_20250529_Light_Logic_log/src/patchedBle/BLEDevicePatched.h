#ifndef _BLE_DEVICE_PATCHED_H_
#define _BLE_DEVICE_PATCHED_H_

#include <Arduino.h>
#include "BLEAdvertPatched.h"
#include "BLEConnectPatched.h"
#include "BLEScanPatched.h"
...
class BLEDevicePatched {
    public:
        ...
        BLEAdvertPatched* configAdvert();
        BLEScanPatched* configScan();
        BLEConnectPatched* configConnection();
        BLESecurityPatched* configSecurity();
        ...
        void addService(BLEServicePatched& newService);
        void configClient();
        BLEClientPatched* addClient(uint8_t connId);
    ...
    private:
        ...
        static BLEAdvertPatched* _pBLEAdvert;  // Pointer to advertisement object (peripheral mode)
        static BLEScanPatched* _pBLEScan;      // Pointer to scan object (central mode)
        static void (*_pScanCB)(T_LE_CB_DATA*); // Pointer to callback function for processing scan data
        static BLEConnectPatched* _pBLEConn;   // Pointer to connect object (central mode)
        static BLESecurityPatched* _pBLESecurity;  // Pointer to security object
        static BLEServicePatched* _servicePtrList[BLE_MAX_SERVICE_COUNT];
        static uint8_t _serviceCount;
        static BLEClientPatched* _clientPtrList[BLE_CENTRAL_APP_MAX_LINKS];
...
extern BLEDevicePatched BLEPatched;
