...
BLEServicePatched::BLEServicePatched(BLEUUIDPatched uuid) {
    setUUID(uuid);
}

BLEServicePatched::BLEServicePatched(const char* uuid) {
    setUUID(BLEUUIDPatched(uuid));
}

BLEServicePatched::~BLEServicePatched() {
    free(_service_attr_tbl);
}

void BLEServicePatched::setUUID(BLEUUIDPatched uuid) {
...
BLEUUIDPatched BLEServicePatched::getUUID() {
    return _uuid;
}

void BLEServicePatched::addCharacteristic(BLECharacteristicPatched& newChar) {
...
BLECharacteristicPatched* BLEServicePatched::getCharacteristic(uint8_t charIndex) {
...
void BLEServicePatched::setServiceID(T_SERVER_ID service_id) {
    _service_id = service_id;
}

T_SERVER_ID BLEServicePatched::getServiceID() {
    return _service_id;
}

// Calculate total number of attributes required by service and characteristics
uint8_t BLEServicePatched::getAttrCount() {
    _total_attr_count = 1;
    uint8_t i;
...
T_ATTRIB_APPL* BLEServicePatched::generateServiceAttrTable() {
...
uint8_t BLEServicePatched::generateAttrServiceDeclaration(T_ATTRIB_APPL* attr_tbl, uint8_t index) {
    if (_uuid.length() == 2) {
...
void BLEServicePatched::printAttr() {
    uint8_t i;
...
T_APP_RESULT BLEServicePatched::serviceAttrReadCallbackDefault(uint8_t conn_id, T_SERVER_ID service_id, uint16_t attrib_index,
...
T_APP_RESULT BLEServicePatched::serviceAttrWriteCallbackDefault(uint8_t conn_id, T_SERVER_ID service_id, uint16_t attrib_index,
...
void BLEServicePatched::serviceCccdUpdateCallbackDefault(uint8_t conn_id, T_SERVER_ID service_id, uint16_t attrib_index, uint16_t ccc_bits) {
    if (service_id != _service_id) {
