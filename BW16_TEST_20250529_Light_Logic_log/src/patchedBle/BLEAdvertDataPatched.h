#ifndef _BLE_ADVERTDATA_PATCHED_H_
#define _BLE_ADVERTDATA_PATCHED_H_
...
class BLEAdvertDataPatched {
    public:
        BLEAdvertDataPatched();
        void clear(void);
        ...
        uint8_t addPartialServices(BLEUUIDPatched uuid);
        uint8_t addCompleteServices(BLEUUIDPatched uuid);
        ...
        BLEAddrPatched     getAddr();
        int8_t      getRSSI();
        uint8_t     getFlags();
        uint8_t     getServiceCount();
        BLEUUIDPatched*    getServiceList();
        String      getName();
        int8_t      getTxPower();
    ...
    private:
        ...
        BLEAddrPatched     _address;
        int8_t      _rssi = 0;
        uint8_t     _flags = 0;
        BLEUUIDPatched     _serviceList[7];        // A 31byte advert can only fit a maximum of 7 service UUIDs of 16bit length
