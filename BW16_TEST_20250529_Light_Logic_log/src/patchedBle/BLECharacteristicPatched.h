#ifndef _BLE_CHARACTERISTIC_PATCHED_H_
#define _BLE_CHARACTERISTIC_PATCHED_H_
...
class BLECharacteristicPatched {
    public:
        BLECharacteristicPatched(BLEUUIDPatched uuid);
        BLECharacteristicPatched(const char* uuid);
        ~BLECharacteristicPatched();
        ...
        //------------- Callbacks -------------//
        void setReadCallback(void (*fCallback) (BLECharacteristicPatched* chr, uint8_t conn_id));              // Called when client reads value
        void setWriteCallback(void (*fCallback) (BLECharacteristicPatched* chr, uint8_t conn_id));             // Called when client writes to value
        void setCCCDCallback(void (*fCallback) (BLECharacteristicPatched* chr, uint8_t conn_id, uint16_t ccc_bits));              // Called when client modifies CCCD
    ...
    private:
        ...
        friend class BLEServicePatched;

        BLEServicePatched* _pService = nullptr;
        ...
        void (*_pReadCB)(BLECharacteristicPatched* chr, uint8_t conn_id) = nullptr;
        void (*_pWriteCB)(BLECharacteristicPatched* chr, uint8_t conn_id) = nullptr;
        void (*_pCccdCB)(BLECharacteristicPatched* chr, uint8_t conn_id, uint16_t ccc_bits) = nullptr;
