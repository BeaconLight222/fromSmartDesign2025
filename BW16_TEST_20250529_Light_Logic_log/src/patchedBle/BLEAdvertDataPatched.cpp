#include "BLEAdvertDataPatched.h"
...
BLEAdvertDataPatched::BLEAdvertDataPatched() {
}

void BLEAdvertDataPatched::clear() {
...
void BLEAdvertDataPatched::addData(const uint8_t* data, uint8_t size) {
...
uint8_t BLEAdvertDataPatched::addFlags(uint8_t flags) {
    uint8_t data[3] = {2, GAP_ADTYPE_FLAGS, flags};
    addData(data, 3);
    return _dataSize;
}

uint8_t BLEAdvertDataPatched::addPartialServices(BLEUUIDPatched uuid) {
...
uint8_t BLEAdvertDataPatched::addCompleteServices(BLEUUIDPatched uuid) {
    _serviceList[_serviceCount++] = (uuid);
    switch (uuid.length()) {
        case 2: {
...
uint8_t BLEAdvertDataPatched::addCompleteServices(uint8_t uuidBitLength) {
...
uint8_t BLEAdvertDataPatched::addAppearance(uint16_t appearance) {
...
uint8_t BLEAdvertDataPatched::addShortName(const char* str) {
    _devName = String(str);
...
uint8_t BLEAdvertDataPatched::addCompleteName(const char* str) {
    _devName = String(str);
    uint8_t length = _devName.length();
        ...
        if ((length > 0x01) && ((pos + length) <= 31)) {
            ...
            switch (type) {
                ...
                case GAP_ADTYPE_16BIT_COMPLETE: {
                    ...
                    while (i >= 2) {
                        _serviceList[_serviceCount++] = (BLEUUIDPatched(p_uuid, 2));
                        p_uuid += 2;
                        i -= 2;
                    }
                    break;
                ...
                case GAP_ADTYPE_32BIT_COMPLETE: {
                    ...
                    while (i >= 4) {
                        _serviceList[_serviceCount++] = (BLEUUIDPatched(p_uuid, 4));
                        p_uuid += 4;
                        i -= 4;
                    }
                    break;
                ...
                case GAP_ADTYPE_128BIT_COMPLETE: {
                    uint8_t *p_uuid = buffer;
                    _serviceList[_serviceCount++] = (BLEUUIDPatched(p_uuid, 16));
                    break;
                }
...
bool BLEAdvertDataPatched::hasFlags() {
    return (_flags > 0);
}

bool BLEAdvertDataPatched::hasUUID() {
    return (_serviceCount > 0);
}

bool BLEAdvertDataPatched::hasName() {
    return ((_devName.length()) > 0);
}

bool BLEAdvertDataPatched::hasManufacturer() {
    return (_manufacturerDataLength > 0);
}

T_GAP_ADV_EVT_TYPE BLEAdvertDataPatched::getAdvType() {
    return _advType;
}

T_GAP_REMOTE_ADDR_TYPE BLEAdvertDataPatched::getAddrType() {
    return _addrType;
}

BLEAddrPatched BLEAdvertDataPatched::getAddr() {
    return _address;
}

int8_t BLEAdvertDataPatched::getRSSI() {
    return _rssi;
}

uint8_t BLEAdvertDataPatched::getFlags() {
    return _flags;
}

uint8_t BLEAdvertDataPatched::getServiceCount() {
    return _serviceCount;
}

BLEUUIDPatched* BLEAdvertDataPatched::getServiceList() {
    return _serviceList;
}

String BLEAdvertDataPatched::getName() {
    return _devName;
}

int8_t BLEAdvertDataPatched::getTxPower() {
    return _txPower;
}

uint16_t BLEAdvertDataPatched::getAppearance() {
    return _devAppearance;
}

uint16_t BLEAdvertDataPatched::getManufacturer() {
    return _manufacturer;
}

uint8_t BLEAdvertDataPatched::getManufacturerDataLength() {
    return _manufacturerDataLength;
}

uint8_t* BLEAdvertDataPatched::getManufacturerData() {
