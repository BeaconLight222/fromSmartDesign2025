...
T_APP_RESULT BLEDevicePatched::gapCallbackDefault(uint8_t cb_type, void *p_cb_data) {
...
void BLEDevicePatched::ioMsgHandlerDefault(T_IO_MSG io_msg) {
...
void BLEDevicePatched::gapMsgHandlerDefault(T_IO_MSG *p_gap_msg) {
...
void BLEDevicePatched::devStateEvtHandlerPeriphDefault(T_GAP_DEV_STATE new_state, uint16_t cause) {
...
void BLEDevicePatched::devStateEvtHandlerCentralDefault(T_GAP_DEV_STATE new_state, uint16_t cause) {
...
void BLEDevicePatched::connStateEvtHandlerPeriphDefault(uint8_t conn_id, T_GAP_CONN_STATE new_state, uint16_t disc_cause) {
...
void BLEDevicePatched::connStateEvtHandlerCentralDefault(uint8_t conn_id, T_GAP_CONN_STATE new_state, uint16_t disc_cause) {
...
void BLEDevicePatched::connMtuInfoEvtHandlerDefault(uint8_t conn_id, uint16_t mtu_size) {
    if (BTDEBUG) printf("connMtuInfoEvtHandlerDefault: conn_id %d, mtu_size %d\r\n", conn_id, mtu_size);
}

void BLEDevicePatched::connParamUpdateEvtHandlerDefault(uint8_t conn_id, uint8_t status, uint16_t cause) {
    switch (status) {
...
void BLEDevicePatched::authenStateEvtHandlerDefault(uint8_t conn_id, uint8_t new_state, uint16_t cause) {
...
T_APP_RESULT BLEDevicePatched::appServiceCallbackDefault(T_SERVER_ID service_id, void *p_data) {
    T_APP_RESULT result = APP_RESULT_SUCCESS;
...
T_APP_RESULT BLEDevicePatched::serviceAttrReadCallbackDefault(uint8_t conn_id, T_SERVER_ID service_id, uint16_t attrib_index,
...
T_APP_RESULT BLEDevicePatched::serviceAttrWriteCallbackDefault(uint8_t conn_id, T_SERVER_ID service_id, uint16_t attrib_index,
...
void BLEDevicePatched::serviceCccdUpdateCallbackDefault(uint8_t conn_id, T_SERVER_ID service_id, uint16_t attrib_index, uint16_t ccc_bits) {
    uint8_t i;
    for (i = 0; i < _serviceCount; i++) {
        if ((_servicePtrList[i]->getServiceID()) == service_id) {
...
T_APP_RESULT BLEDevicePatched::appClientCallbackDefault(T_CLIENT_ID client_id, uint8_t conn_id, void *p_data) {
    T_APP_RESULT  result = APP_RESULT_SUCCESS;
...
void BLEDevicePatched::clientDiscoverStateCallbackDefault(uint8_t conn_id, T_DISCOVERY_STATE discovery_state) {
...
void BLEDevicePatched::clientDiscoverResultCallbackDefault(uint8_t conn_id, T_DISCOVERY_RESULT_TYPE result_type, T_DISCOVERY_RESULT_DATA result_data) {
    if (_clientPtrList[conn_id] == nullptr) {
        return;
    }
...
void BLEDevicePatched::clientReadResultCallbackDefault(uint8_t conn_id, uint16_t cause, uint16_t handle, uint16_t value_size, uint8_t *p_value) {
    if (_clientPtrList[conn_id] == nullptr) {
        return;
    }
...
void BLEDevicePatched::clientWriteResultCallbackDefault(uint8_t conn_id, T_GATT_WRITE_TYPE type, uint16_t handle, uint16_t cause, uint8_t credits) {
    if (_clientPtrList[conn_id] == nullptr) {
        return;
    }
...
T_APP_RESULT BLEDevicePatched::clientNotifyIndicateCallbackDefault(uint8_t conn_id, bool notify, uint16_t handle, uint16_t value_size, uint8_t *p_value) {
...
void BLEDevicePatched::clientDisconnectCallbackDefault(uint8_t conn_id) {
    if (_clientPtrList[conn_id] == nullptr) {
        return;
    }
