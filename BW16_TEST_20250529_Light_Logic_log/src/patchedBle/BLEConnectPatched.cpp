#include "BLEConnectPatched.h"
#include "BLEDevicePatched.h"
#include "BLEAddrPatched.h"
...
BLEConnectPatched::BLEConnectPatched() {
...
bool BLEConnectPatched::connect(char* btAddr, T_GAP_REMOTE_ADDR_TYPE destAddrType, uint16_t scanTimeout) {
    BLEAddrPatched destAddr(btAddr);
    return connect(destAddr, destAddrType, scanTimeout);
}

bool BLEConnectPatched::connect(uint8_t (&btAddr)[6], T_GAP_REMOTE_ADDR_TYPE destAddrType, uint16_t scanTimeout) {
    BLEAddrPatched destAddr(btAddr);
    return connect(destAddr, destAddrType, scanTimeout);
}

bool BLEConnectPatched::connect(BLEAdvertDataPatched targetDevice, uint16_t scanTimeout) {
    BLEAddrPatched destAddr = targetDevice.getAddr();
    T_GAP_REMOTE_ADDR_TYPE destAddrType = targetDevice.getAddrType();
    return connect(destAddr, destAddrType, scanTimeout);
}

bool BLEConnectPatched::connect(BLEAddrPatched destAddr, T_GAP_REMOTE_ADDR_TYPE destAddrType, uint16_t scanTimeout) {
...
bool BLEConnectPatched::disconnect(uint8_t connId) {
...
void BLEConnectPatched::setScanInterval(uint16_t scanInt_ms) {
    if((scanInt_ms >= 3) && (scanInt_ms <= 10240)) {
        _connReqParam.scan_interval = (scanInt_ms*1000/625);
    }
}

void BLEConnectPatched::setScanWindow(uint16_t scanWindow_ms) {
...
void BLEConnectPatched::setConnInterval(uint16_t min_ms, uint16_t max_ms) {
...
void BLEConnectPatched::setConnLatency(uint16_t latency) {
    if (latency > 0x01F3) {
...
void BLEConnectPatched::setConnTimeout(uint16_t timeout_ms) {
...
void BLEConnectPatched::updateConnParams(uint8_t conn_id) {
...
bool BLEConnectPatched::getConnInfo(uint8_t connId, T_GAP_CONN_INFO *pConnInfo) {
    return le_get_conn_info(connId, pConnInfo);
}

bool BLEConnectPatched::getConnAddr(uint8_t connId, uint8_t* addr, uint8_t* addrType) {
    return le_get_conn_addr(connId, addr, addrType);
}

int8_t BLEConnectPatched::getConnId(char* btAddr, uint8_t addrType) {
    uint8_t connId = 0;
    BLEAddrPatched destAddr(btAddr);
...
int8_t BLEConnectPatched::getConnId(uint8_t* btAddr, uint8_t addrType) {
    uint8_t connId = 0;
...
int8_t BLEConnectPatched::getConnId(BLEAdvertDataPatched targetDevice) {
    uint8_t connId = 0;
