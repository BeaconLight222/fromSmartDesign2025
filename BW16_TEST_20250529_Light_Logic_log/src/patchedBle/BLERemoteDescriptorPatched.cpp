#include "BLERemoteDescriptorPatched.h"
...
BLEUUIDPatched BLERemoteDescriptorPatched::getUUID() {
    return _uuid;
}

void BLERemoteDescriptorPatched::setBufferLen(uint16_t max_len) {
...
uint16_t BLERemoteDescriptorPatched::getBufferLen() {
    return _data_buf_len;
}

//--------- Read Char Value --------//

String BLERemoteDescriptorPatched::readString() {
    char datastring[_data_buf_len + 1] = {0};
    getData((uint8_t*)datastring, sizeof(datastring));
    return String(datastring);
}

uint8_t BLERemoteDescriptorPatched::readData8() {
    uint8_t num = 0;
    getData(&num, sizeof(num));
    return num;
}

uint16_t BLERemoteDescriptorPatched::readData16() {
    uint16_t num = 0;
    getData((uint8_t*) &num, sizeof(num));
    return num;
}

uint32_t BLERemoteDescriptorPatched::readData32() {
    uint32_t num = 0;
    getData((uint8_t*) &num, sizeof(num));
    return num;
}
...
bool BLERemoteDescriptorPatched::writeString(String str) {
    return writeString(str.c_str());
}

bool BLERemoteDescriptorPatched::writeString(const char* str) {
    return setData((uint8_t*) str, strlen(str));
}

bool BLERemoteDescriptorPatched::writeData8(uint8_t num) {
    return setData((uint8_t*) &num, sizeof(num));
}

bool BLERemoteDescriptorPatched::writeData16(uint16_t num) {
    return setData((uint8_t*) &num, sizeof(num));
}

bool BLERemoteDescriptorPatched::writeData32(uint32_t num) {
    return setData((uint8_t*) &num, sizeof(num));
}

bool BLERemoteDescriptorPatched::writeData32(int num) {
    return setData((uint8_t*) &num, sizeof(num));
}

//------------ Modify Char ------------//

bool BLERemoteDescriptorPatched::setData(uint8_t* data, uint16_t datalen) {
...
uint16_t BLERemoteDescriptorPatched::getData(uint8_t* data, uint16_t datalen) {
    // Check if device is still connected
    if (!_pClient->connected()) {
        printf("Descriptor %s error: client not connected \n", _uuid.str());
        return 0;
...
BLERemoteDescriptorPatched::BLERemoteDescriptorPatched(BLEUUIDPatched uuid) {
    _uuid = uuid;
...
BLERemoteDescriptorPatched::~BLERemoteDescriptorPatched() {
    free(_data_buf);
    vSemaphoreDelete(_readSemaphore);
    vSemaphoreDelete(_writeSemaphore);
}

void BLERemoteDescriptorPatched::clientReadResultCallbackDefault(uint8_t conn_id, uint16_t cause, uint16_t handle, uint16_t value_size, uint8_t *p_value) {
...
void BLERemoteDescriptorPatched::clientWriteResultCallbackDefault(uint8_t conn_id, T_GATT_WRITE_TYPE type, uint16_t handle, uint16_t cause, uint8_t credits) {
    if (handle == _declarationHandle) {
        if (cause == GAP_SUCCESS) {
