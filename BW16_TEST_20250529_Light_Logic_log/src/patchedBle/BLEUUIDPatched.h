...
#ifndef _BLE_UUID_PATCHED_H_
#define _BLE_UUID_PATCHED_H_

#include <Arduino.h>

#define BLE_UUID_MAX_LENGTH 16

class BLEUUIDPatched {
    public: 
        BLEUUIDPatched();
        BLEUUIDPatched(const char* str);      // Build a BLEUUIDPatched object from an UUID string. Use of hyphens (-) to separate UUID sections in the string is acceptable.
        BLEUUIDPatched(uint8_t* data, uint8_t length);
        const char* str();
        const uint8_t* data();
        const uint8_t* dataNative();
        uint8_t length();
        bool operator ==(const BLEUUIDPatched &uuid);
