...
BLEAdvertPatched::BLEAdvertPatched() {
}

void BLEAdvertPatched::setAdvType(uint8_t advType) {
    if (advType <= GAP_ADTYPE_ADV_LDC_DIRECT_IND) {
        _advEvtType = advType;
    }
}

void BLEAdvertPatched::setMinInterval(uint16_t minInt_ms) {
    if ((minInt_ms >= 20) && (minInt_ms <= 10240)) {
        _advIntMin = (minInt_ms*1000/625);
    }
}

void BLEAdvertPatched::setMaxInterval(uint16_t maxInt_ms) {
    if ((maxInt_ms >= 20) && (maxInt_ms <= 10240)) {
        _advIntMax = (maxInt_ms*1000/625);
    }
}

void BLEAdvertPatched::setAdvData(BLEAdvertDataPatched adData) {
    memcpy(_advData, adData._data, adData._dataSize);
    _advDataSize = adData._dataSize;
}

void BLEAdvertPatched::setScanRspData(BLEAdvertDataPatched adData) {
    memcpy(_scanRspData, adData._data, adData._dataSize);
    _scanRspDataSize = adData._dataSize;
}

void BLEAdvertPatched::setAdvData(uint8_t* pdata, uint8_t size) {
    if (size <= 31) {
...
void BLEAdvertPatched::setScanRspData(uint8_t* pdata, uint8_t size) {
    if (size <= 31) {
...
void BLEAdvertPatched::updateAdvertParams() {
...
void BLEAdvertPatched::startAdv() {
    le_adv_start();
}

void BLEAdvertPatched::stopAdv() {
    le_adv_stop();
}