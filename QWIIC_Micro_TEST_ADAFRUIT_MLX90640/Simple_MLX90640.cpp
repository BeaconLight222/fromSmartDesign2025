...
boolean Simple_MLX90640::begin(uint8_t _i2c_addr) {
  ...
  for (int i = 0; i < 832; i++) {
    SerialUSB.print(eeMLX90640[i], HEX);
    SerialUSB.print(' ');
  }
  SerialUSB.println();
#endif
...
int Simple_MLX90640::MLX90640_I2CRead(uint8_t slaveAddr, uint16_t startAddress,
  ...
  while (nMemAddressRead > 0) {
    uint16_t toRead16 =
      min(nMemAddressRead, (uint16_t)(WIRE_ACCESS_LEN / 2));

    cmd[0] = startAddress >> 8;
    cmd[1] = startAddress & 0x00FF;
    // SerialUSB.print("toRead16 ");
    // SerialUSB.println(toRead16);
    Wire.beginTransmission(slaveAddr);
    Wire.write((startAddress >> 8) & 0xFF);  // MSB
    Wire.write(startAddress & 0xFF);         // LSB
    Wire.endTransmission(false);             // Send repeated start

    Wire.requestFrom(slaveAddr, toRead16 * 2);
...
#ifdef MLX90640_DEBUG
  SerialUSB.print("kVdd: ");
  SerialUSB.println(kVdd);
  SerialUSB.print("vdd25: ");
  SerialUSB.println(vdd25);
#endif
}
...
#ifdef MLX90640_DEBUG
  SerialUSB.print("KvPTAT: ");
  SerialUSB.println(KvPTAT, 8);
  SerialUSB.print("KtPTAT: ");
  SerialUSB.println(KtPTAT);
  SerialUSB.print("vPTAT25: ");
  SerialUSB.println(vPTAT25);
  SerialUSB.print("alphaPTAT: ");
  SerialUSB.println(alphaPTAT);
#endif
}
...
#ifdef MLX90640_DEBUG
  SerialUSB.print("gainEE: ");
  SerialUSB.println(gainEE);
#endif
}
...
#ifdef MLX90640_DEBUG
  SerialUSB.print("ResolutionEE: ");
  SerialUSB.println(resolutionEE);
#endif
}
...
void ExtractKsTaParameters(uint16_t *eeData, paramsMLX90640 *mlx90640) {
  ...
  mlx90640->KsTa = KsTa;
#ifdef MLX90640_DEBUG
  SerialUSB.print("KsTa: ");
  SerialUSB.println(KsTa, 8);
#endif
}
...
#ifdef MLX90640_DEBUG
  SerialUSB.print("KsTo: ");
  for (int i = 0; i < 5; i++) {
    SerialUSB.print(mlx90640->ksTo[i], 8);
    SerialUSB.print(", ");
  }
  SerialUSB.println();
  SerialUSB.print("ct: ");
  for (int i = 0; i < 5; i++) {
    SerialUSB.print(mlx90640->ct[i]);
    SerialUSB.print(", ");
  }
  SerialUSB.println();
#endif
...
void ExtractAlphaParameters(uint16_t *eeData, paramsMLX90640 *mlx90640) {
...
#ifdef MLX90640_DEBUG
  SerialUSB.print("scratchData: ");
  for (int i = 0; i < 768; i++) {
    SerialUSB.print(scratchData[i], 8);
    SerialUSB.print(", ");
  }
  SerialUSB.println();
#endif
  ...
  for (int i = 0; i < 768; i++) {
    SerialUSB.print(mlx90640->alpha[i]);
    SerialUSB.print(", ");
  }
  SerialUSB.println();
  SerialUSB.print("alphascale: ");
  SerialUSB.println(alphaScale);
#endif
}
...
void ExtractOffsetParameters(uint16_t *eeData, paramsMLX90640 *mlx90640) {
  ...
  for (int i = 0; i < 768; i++) {
    SerialUSB.print(mlx90640->offset[i]);
    SerialUSB.print(", ");
  }
  SerialUSB.println();
#endif
...
void ExtractKtaPixelParameters(uint16_t *eeData, paramsMLX90640 *mlx90640) {
  ...
  SerialUSB.print("kta: ");
  for (int i = 0; i < 768; i++) {
    SerialUSB.print(mlx90640->kta[i]);
    SerialUSB.print(", ");
  }
  SerialUSB.println();
  SerialUSB.print("ktascale: ");
  SerialUSB.println(ktaScale1);
#endif
}
...
void ExtractKvPixelParameters(uint16_t *eeData, paramsMLX90640 *mlx90640) {
  ...
  SerialUSB.print("kv: ");
  for (int i = 0; i < 768; i++) {
    SerialUSB.print(mlx90640->kv[i]);
    SerialUSB.print(", ");
  }
  SerialUSB.println();
  SerialUSB.print("kvscale: ");
  SerialUSB.println(kvScale);
#endif
}
...
#ifdef MLX90640_DEBUG
  SerialUSB.print("cpAlpha: ");
  SerialUSB.print(alphaSP[0], 12);
  SerialUSB.print(", ");
  SerialUSB.println(alphaSP[1], 12);
  SerialUSB.print("offsetSP: ");
  SerialUSB.print(offsetSP[0]);
  SerialUSB.print(", ");
  SerialUSB.println(offsetSP[1]);
#endif
}
...
#ifdef MLX90640_DEBUG
  SerialUSB.print("calibEE: ");
  SerialUSB.println(calibrationModeEE);
  SerialUSB.print("ilChess: ");
  SerialUSB.print(ilChessC[0]);
  SerialUSB.print(", ");
  SerialUSB.print(ilChessC[1]);
  SerialUSB.print(", ");
  SerialUSB.println(ilChessC[2]);
#endif
}
...
int ExtractDeviatingPixels(uint16_t *eeData, paramsMLX90640 *mlx90640) {
  ...
  if (brokenPixCnt > 4) {
    SerialUSB.print("Broken pixels: ");
    SerialUSB.println(brokenPixCnt);
    warn = -3;
  } else if (outlierPixCnt > 4) {
    SerialUSB.print("Outlier pixels: ");
    SerialUSB.println(outlierPixCnt);
    warn = -4;
  } else if ((brokenPixCnt + outlierPixCnt) > 4) {
    SerialUSB.print("Broken+outlier pixels: ");
    SerialUSB.println(brokenPixCnt + outlierPixCnt);
    warn = -5;
  } else {
    for (pixCnt = 0; pixCnt < brokenPixCnt; pixCnt++) {
      for (i = pixCnt + 1; i < brokenPixCnt; i++) {
        ...
        if (warn != 0) {
          SerialUSB.println("Broken pixel has adjacent broken pixel");
          return warn;
        }
      }
    }
    ...
    for (pixCnt = 0; pixCnt < outlierPixCnt; pixCnt++) {
      for (i = pixCnt + 1; i < outlierPixCnt; i++) {
        ...
        if (warn != 0) {
          SerialUSB.println("Outlier pixel has adjacent outlier pixel");
          return warn;
        }
      }
    }
    ...
    for (pixCnt = 0; pixCnt < brokenPixCnt; pixCnt++) {
      for (i = 0; i < outlierPixCnt; i++) {
        ...
        if (warn != 0) {
          SerialUSB.println("Broken pixel has adjacent outlier pixel");
          return warn;
        }
      }
    }
