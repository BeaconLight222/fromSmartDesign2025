...
//#define MLX90640_DEBUG 1

//on QWIIC MICRO it seems to be 256
...
Simple_MLX90640::Simple_MLX90640() {
  // Constructor implementation (if needed)
}

boolean Simple_MLX90640::begin(uint8_t _i2c_addr) {
  i2c_addr = _i2c_addr;

  // We are using PA25 and PA26 for I2C
  ...
  int readNumber = MLX90640_I2CRead(i2c_addr, MLX90640_DEVICEID1, 3, serialNumber);
  ...
  for (int i = 0; i < 832; i++) {
    Serial.print(eeMLX90640[i], HEX);
    Serial.print(' ');
  }
  Serial.println();
#endif

  uint8_t flashSwap[768*2+768*2+768+768];
  _params.alpha = (uint16_t*)(flashSwap+0);  // Assign the alpha array to the params structure
  _params.offset = (int16_t*)(flashSwap+768*2);  // Assign the offset array to the params structure
  _params.kta = (int8_t*)(flashSwap+768*2+768*2);  // Assign the kta array to the params structure
  _params.kv = (int8_t*)(flashSwap+768*2+768*2+768);  // Assign the kv array to the params structure
  MLX90640_ExtractParameters(eeMLX90640, &_params);

  int irSensorBaseAddress = 0x08205000; //use Wi-Fi Fast Connect profile space, we are not using it
  bool contentSame = (memcmp((void*)irSensorBaseAddress, (void*)flashSwap, sizeof(flashSwap)) == 0);
  if (!contentSame) {
    Serial.println("IR sensor EEPROM data is not in flash memory or different, writing to flash...");
    // Write the EEPROM data to flash memory
    int flashAddress = irSensorBaseAddress&0xFFFFFF;
    flash_erase_sector(NULL,(uint32_t)flashAddress);
    flash_stream_write(NULL,(uint32_t)flashAddress, sizeof(flashSwap), flashSwap);
  } else {
    Serial.println("IR sensor EEPROM data is already in flash memory and same, skipping write.");
  }

  _params.alpha = (uint16_t*)(irSensorBaseAddress + 0x00);  // Assign the alpha array to the params structure
  _params.offset = (int16_t*)(irSensorBaseAddress + 0x00 + 768*2);  // Assign the offset array to the params structure
  _params.kta = (int8_t*)(irSensorBaseAddress + 0x00 + 768*2 + 768*2);  // Assign the kta array to the params structure
  _params.kv = (int8_t*)(irSensorBaseAddress + 0x00 + 768*2 + 768*2 + 768);  // Assign the kv array to the params structure 
...
int Simple_MLX90640::MLX90640_I2CRead(uint8_t slaveAddr, uint16_t startAddress,
                                      uint16_t nMemAddressRead, uint16_t *data) {

  // uint8_t cmd[2];

  while (nMemAddressRead > 0) {
    uint16_t toRead16 = nMemAddressRead;
    if (toRead16 > (uint16_t)(WIRE_ACCESS_LEN / 2)) {
      toRead16 = (uint16_t)(WIRE_ACCESS_LEN / 2);
    }

    // cmd[0] = startAddress >> 8;
    // cmd[1] = startAddress & 0x00FF;
    // Serial.print("toRead16 ");
    // Serial.println(toRead16);
    Wire.beginTransmission(slaveAddr);
    Wire.write((startAddress >> 8) & 0xFF);  // MSB
    Wire.write(startAddress & 0xFF);         // LSB
    Wire.endTransmission(false);             // Send repeated start
    //https://github.com/Ameba-AIoT/ameba-arduino-d/blob/22486c9f94c1db52c070f62b3281c50174321145/Arduino_package/hardware/libraries/Wire/src/Wire.cpp#L187
    //void i2c_restart_enable(i2c_t *obj) may be needed

    Wire.requestFrom(slaveAddr, (uint8_t)(toRead16 * 2));

    for (uint16_t i = 0; i < toRead16; i++) {
      if (Wire.available() < 2) {
        return -1;  // Not enough data available
      }
      data[i] = (Wire.read() << 8) | Wire.read();  // Read two bytes and combine them
    }
...
int Simple_MLX90640::MLX90640_I2CWrite(uint8_t slaveAddr, uint16_t writeAddress,
                                       uint16_t data) {
  uint16_t datacheck = 0;
  Wire.beginTransmission(slaveAddr);
  Wire.write((writeAddress >> 8) & 0xFF);  // MSB
  ...
  delay(1);
  //read back the register to check if it was written correctly
...
int Simple_MLX90640::MLX90640_DumpEE(uint8_t slaveAddr, uint16_t *eeData) {
  // Read the EEPROM data from the device
  return MLX90640_I2CRead(slaveAddr, 0x2400, 832, eeData);
}

int Simple_MLX90640::MLX90640_ExtractParameters(uint16_t *eeData, paramsMLX90640 *mlx90640) {
  int error = 0;
...
mlx90640_mode_t Simple_MLX90640::getMode(void) {
  return (mlx90640_mode_t)MLX90640_GetCurMode(i2c_addr);
}

void Simple_MLX90640::setMode(mlx90640_mode_t mode) {
  if (mode == MLX90640_CHESS) {
    MLX90640_SetChessMode(i2c_addr);
  } else {
    MLX90640_SetInterleavedMode(i2c_addr);
  }
}

int Simple_MLX90640::MLX90640_GetCurMode(uint8_t slaveAddr) {
...
int Simple_MLX90640::MLX90640_SetInterleavedMode(uint8_t slaveAddr) {
  uint16_t controlRegister1;
  int value;
  int error;
...
int Simple_MLX90640::MLX90640_SetChessMode(uint8_t slaveAddr) {
  uint16_t controlRegister1;
  int value;
  int error;
...
mlx90640_resolution_t Simple_MLX90640::getResolution(void) {
  return (mlx90640_resolution_t)MLX90640_GetCurResolution(i2c_addr);
}
 ...
 */
void Simple_MLX90640::setResolution(mlx90640_resolution_t res) {
  MLX90640_SetResolution(i2c_addr, (int)res);
}

int Simple_MLX90640::MLX90640_SetResolution(uint8_t slaveAddr, uint8_t resolution)
{
    uint16_t controlRegister1;
    int value;
    int error;
...
int Simple_MLX90640::MLX90640_GetCurResolution(uint8_t slaveAddr)
{
    uint16_t controlRegister1;
    int resolutionRAM;
    int error;
...
mlx90640_refreshrate_t Simple_MLX90640::getRefreshRate(void) {
  return (mlx90640_refreshrate_t)MLX90640_GetRefreshRate(i2c_addr);
}
 ...
 */
void Simple_MLX90640::setRefreshRate(mlx90640_refreshrate_t rate) {
  MLX90640_SetRefreshRate(i2c_addr, (int)rate);
}

int Simple_MLX90640::MLX90640_SetRefreshRate(uint8_t slaveAddr, uint8_t refreshRate)
{
    uint16_t controlRegister1;
    int value;
    int error;
...
int Simple_MLX90640::MLX90640_GetRefreshRate(uint8_t slaveAddr)
{
    uint16_t controlRegister1;
    int refreshRate;
    int error;
...
int Simple_MLX90640::getFrame(float *framebuf) {
  ...
  for (uint8_t page = 0; page < 2; page++) {
    status = MLX90640_GetFrameData(i2c_addr, mlx90640Frame);
...
float Simple_MLX90640::getTa(bool newFrame) {
  if (!newFrame) {
    return ta;
  }
  uint16_t mlx90640Frame[834];
  MLX90640_GetFrameData(i2c_addr, mlx90640Frame);
  return MLX90640_GetTa(mlx90640Frame, &_params);
}

int Simple_MLX90640::MLX90640_GetFrameData(uint8_t slaveAddr, uint16_t *frameData)
{
...
float Simple_MLX90640::MLX90640_GetTa(uint16_t *frameData, const paramsMLX90640 *params)
{
    float ptat;
    float ptatArt;
    float vdd;
...
float Simple_MLX90640::MLX90640_GetVdd(uint16_t *frameData, const paramsMLX90640 *params)
{
    float vdd;
...
void Simple_MLX90640::MLX90640_CalculateTo(uint16_t *frameData, const paramsMLX90640 *params, float emissivity, float tr, float *result)
{
    float vdd;
    float ta;
    float ta4;
