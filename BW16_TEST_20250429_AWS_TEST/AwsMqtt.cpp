...
#include "ModifiedWifiClient.h"  // use the modified wifi client to avoid the "WiFiClient is not a class or namespace" error

#define ARDUINOJSON_ENABLE_STD_STREAM 0
#include "src/ArduinoJson-7.4.1/ArduinoJson-v7.4.1.h"

ModifiedWifiSSLClient wifiClient;
...
//char mqttServer[] = "ado4y320oe6ti-ats.iot.us-east-1.amazonaws.com";  //tad test 20250522
char* clientId;
...
const char* jobStateToString(int state) {
  switch (state) {
    case WAIT_FOR_CONNECTION: return "WAIT_FOR_CONNECTION";
    case CONNECTED: return "CONNECTED";
    case READY_TO_SEND_INIT_JOB_QUERY: return "READY_TO_SEND_INIT_JOB_QUERY";
    case WAIT_FOR_JOB_QUERY_RESPONSE: return "WAIT_FOR_JOB_QUERY_RESPONSE";
    case IDLE: return "IDLE";
    case REQUEST_JOB_DETAIL: return "REQUEST_JOB_DETAIL";
    case WAIT_FOR_JOB_DETAIL: return "WAIT_FOR_JOB_DETAIL";
    case DO_THE_JOB: return "DO_THE_JOB";
    case DO_THE_OTA_JOB: return "DO_THE_OTA_JOB";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (awsMqttPtr != NULL) {
    ...
    String streamTopicPrefix = String(awsPrefix) + String(thingName) + "/streams/";
...
void AwsMqtt::begin() {
  ...
  otaProcessor.initCodeSigningCert((char*)NonvolatileDataManager::getFirmwareSignPubKey());
...
void AwsMqtt::reconnect() {
  ...
  while (!client.connected()) {
    ...
    wifiClient.setRootCAPreParsed(NonvolatileDataManager::getCaCertificatePreParsed());
    wifiClient.setClientCertificate(NonvolatileDataManager::getCertificatePreParsed(),
    ...
    if (client.connect(clientId)) {
      ...
      //test tad
      // {
      //   String commandSub = String(thingName) + "/command/sub";
      //   client.subscribe(commandSub.c_str());
      //   Serial.print("Subscribe :");
      //   Serial.println(commandSub.c_str());
      //   String deviceConfigSub = String(thingName) + "/deviceConfig/sub";
      //   client.subscribe(deviceConfigSub.c_str());
      //   Serial.print("Subscribe :");
      //   Serial.println(deviceConfigSub.c_str());

      //   client.publish(commandSub.c_str(), "{\"status\":\"hello from beacon\"}");
      //   Serial.print("Publish :");
      //   Serial.println(commandSub.c_str());
      // }

      //if we do not do any subscribe or publish here, the client.loop() takes 100s for the first time
      // seems to be printf issue.
...
void AwsMqtt::loop() {
  ...
  int pastMillis = (int)(millis() - lastDelay);
  if (pastMillis < 100) {
    delay(100 - pastMillis); //just for LED to be visible
  }
  lastDelay = millis();
    ...
    case WAIT_FOR_JOB_QUERY_RESPONSE:
      {
        ...
        for (int i = (int)(jobsMessages.size()) - 1; i >= 0; i--) {
          if (strstr(jobsMessages[i].topic.c_str(), "/jobs/get/accepted") != NULL) {
            JsonDocument doc;
            DeserializationError error = deserializeJson(doc, jobsMessages[i].payload.c_str(), jobsMessages[i].payload.length());
            if (!error) {
    ...
    case REQUEST_JOB_DETAIL:
      {
        ...
        String getAcceptedTopic = String(awsPrefix) + String(thingName) + "/jobs/" + processingJobName + "/get/accepted";
        client.subscribe(getAcceptedTopic.c_str());
        Serial.print("Subscribe :");
        Serial.println(getAcceptedTopic.c_str());
        //publish the job detail request
        String getJobTopic = String(awsPrefix) + String(thingName) + "/jobs/" + processingJobName + "/get";
        client.publish(getJobTopic.c_str(), "{}");
        Serial.print("Publish :");
        Serial.println(getJobTopic.c_str());
    ...
    case WAIT_FOR_JOB_DETAIL:
      {
        ...
        String getAcceptedTopic = String(awsPrefix) + String(thingName) + "/jobs/" + processingJobName + "/get/accepted";
        for (int i = (int)(jobsMessages.size()) - 1; i >= 0; i--) {
          if (strstr(jobsMessages[i].topic.c_str(), getAcceptedTopic.c_str()) != NULL) {
            JsonDocument doc;
            DeserializationError error = deserializeJson(doc, jobsMessages[i].payload.c_str(), jobsMessages[i].payload.length());
            if (!error) {
            ...
            //remove the message from the list
            jobsMessages.erase(jobsMessages.begin() + i);
            String updateTopic = String(awsPrefix) + String(thingName) + "/jobs/" + processingJobName + "/update";
    ...
    case DO_THE_JOB:
      {
        ...
        DeserializationError error = deserializeJson(jobContentDoc, processingJobContent.c_str(), processingJobContent.length());
        if (!error) {
                ...
                if (protocol.is<const char*>()) {
                  ...
                  if (strcmp(protocolStr, "MQTT") == 0) {
                    ...
                    if (afrOta["streamname"].is<const char*>()) {
                      processingOTAStreamName = afrOta["streamname"].as<const char*>();
                      ...
                      for (JsonVariant file : files) {
                        if (file.is<JsonObject>()) {
                          ...
                          if (fileObj["sig-sha256-ecdsa"].is<const char*>()) {
                            processingOTASignature = fileObj["sig-sha256-ecdsa"].as<const char*>();
        ...
        if (processingOTAFileSize > 0 && processingOTAStreamName.length() > 0 && processingOTASignature.length() > 0) {
          //OTA job
    ...
    case DO_THE_OTA_JOB:
      {
        ...
        if ((processingOTAReadedBytes == 0) && (processingOTARequestedBytes == 0)) {
          ...
          String jsonDataTopic = String(awsPrefix) + String(thingName) + "/streams/" + processingOTAStreamName + "/data/json";
          client.subscribe(jsonDataTopic.c_str());
          Serial.print("OTA job subscribe topic: ");
          Serial.println(jsonDataTopic.c_str());
          String jsonDataRejectedTopic = String(awsPrefix) + String(thingName) + "/streams/" + processingOTAStreamName + "/rejected/json";
        ...
        if (processingOTAReadedBytes < processingOTARequestedBytes) {
          ...
          for (int i = (int)(jobsMessages.size()) - 1; i >= 0; i--) {
            if (strstr(jobsMessages[i].topic.c_str(), jsonDataTopic.c_str()) != NULL) {
              JsonDocument doc;
              DeserializationError error = deserializeJson(doc, jobsMessages[i].payload.c_str(), jobsMessages[i].payload.length());
              if (!error) {
                int dataLen = 0;
                ...
                if (dataLen > 0 && dataPayload != NULL && blockIndex >= 0) {
                  otaProcessor.OtaProcessDataChunkBase64((char*)dataPayload, dataLen, blockIndex * requestBytes);
                }else{
              ...
              jobsMessages.erase(jobsMessages.begin() + i);
            }else if (strstr(jobsMessages[i].topic.c_str(), jsonDataRejectedTopic.c_str()) != NULL) {
          ...
          if (processingOTAReadedBytes == processingOTAFileSize) {
            ...
            endOta = 1;
            if (otaProcessor.OtaProcessVerifySignature((char *)processingOTASignature.c_str()) == 0) {
            ...
            String jsonDataRequestTopic = String(awsPrefix) + String(thingName) + "/streams/" + processingOTAStreamName + "/get/json";
            ...
            String getStreamRequest = "{\"f\": " + String(idOfStream) + ", \"l\": " + String(payloadLength) + ", \"o\": " + String(offset) + ", \"n\": " + String(numberOfBlocks) + "}";
        ...
        if (endOta != 0) {
          ...
          Serial.print("OTA job unsubscribe topic: ");
          Serial.println(jsonDataRejectedTopic.c_str());
          String jobUpdateTopic = String(awsPrefix) + String(thingName) + "/jobs/" + processingJobName + "/update";
