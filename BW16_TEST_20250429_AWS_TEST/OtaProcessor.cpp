...
int OtaProcessor::OtaEnableOTA(){
    ...
    if (currentImageNumber == 1){
        ...
        Serial.println("currentImageNumber is not set, please call getCurrentImgNumber() first");
        return 0;
    }
...
int OtaProcessor::OtaProcessDataChunk(uint8_t* data, size_t length, size_t offset){
    ...
    if (offset == 0){
        memset(pageSwapData, 0, 8); // do not enable image until signature is verified
    }
