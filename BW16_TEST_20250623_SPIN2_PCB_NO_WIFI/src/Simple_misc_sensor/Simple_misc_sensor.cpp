...
boolean Simple_TMP102::begin(){
    ...
    // 0x02: TLOW register
    // 0x03: THIGH register
    // there is no signature register, so we cannot check if the device is a TMP102
    // Assume it is.
...
float Simple_TMP102::checkTemperatureData() {
    ...
    int16_t rawTemp = (Wire.read() << 4) | (Wire.read() >> 4);  // Read the two bytes
...
bool Simple_DS3231::write_then_read(const uint8_t *write_buffer, int write_len, uint8_t *read_buffer, int read_len, bool stop){
    Wire.beginTransmission(DS3231_ADDRESS);
...
void Simple_DS3231::clearLostPowerFlag(void) {
    ...
    write_then_read(writeBuffer, 2, writeBuffer, 0, true);  // Write back the modified status register
}
...
int Simple_EEPROM_AT24C08::read(uint16_t memAddress, uint8_t *data, uint16_t length) {
    ...
    while (length > 0) {
        ...
        uint8_t readAddress = EEPROM_START_ADDRESS + (memAddress >> 8);  // High byte of address

        // Set the memory address to read from
        Wire.beginTransmission(readAddress);
        Wire.write((uint8_t)(memAddress & 0xFF));  // Low byte only as high byte is in the address
...
int Simple_EEPROM_AT24C08::write(uint16_t memAddress, uint8_t *data, uint16_t length) {
    if (length == 0 || data == nullptr) {
    ...
    while (length > 0) {
        ...
        uint8_t writeAddress = EEPROM_START_ADDRESS + (memAddress >> 8);  // High byte of address
