...
const struct Anchor k_anchors[5] = {
...
//     int validDetections = post_nms(detections, detectionsCount, nmsThreshold);    // suppress by setting confidence to zero

//     TM_PRINTF("validDetections %d\n", validDetections);
//     for (int i=0; i<validDetections; i++){
//         TM_PRINTF("valid detection %d: x %.2f y %.2f w %.2f h %.2f conf %.2f\n", i, detections[i].x, detections[i].y, detections[i].w, detections[i].h, detections[i].conf);
//     }


//     char filename[128];
//     sprintf(filename, "debug/c_gen_detection_%03d.txt", outputCounterForDebug);
...
//         background_update(scaledData, detections, 0, backgroundEmaDecay, thresholdForBackgroundSub, backgroundData);
//     }
...
//     // pre-process input
//     for (int i=0; i<(24*32); i++) {
//         const int32_t value = (scaledData[i] - backgroundData[i]) * MODEL_INPUT_QUANT_SCALE + MODEL_INPUT_QUANT_ZERO;
...
void feedFloatMLX90640YoloProcessorAndRun(float *image){
    ...
    validDetections = post_nms(detections, detectionsCount, nmsThreshold);    // suppress by setting confidence to zero
    ...
    if (backgroundUpdateCount >= backgroundUpdateSteps) {
        //printf("background update\n");
        backgroundUpdateCount = 0;
        background_update(scaledData, detections, validDetections, backgroundEmaDecay, thresholdForBackgroundSub, backgroundData);
    }
